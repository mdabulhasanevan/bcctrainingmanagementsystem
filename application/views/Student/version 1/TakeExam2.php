...
.myTimediv{position:fixed; top:180px; right: 50px;}
.MobileTimer{display:none;}

@media (max-width: 600px) { 
.myTimediv{display:none;}
.MobileTimer{display:block; position:fixed; top:180px; right: 5px; background-color:#D5FE52; color:black; border:1px solid black; padding: 3px;}
...
<div  ng-controller="CreateQuestionCtrl"  class="col-md-12" style="background-color: #ffffff;">
      ...
      id="fullScreen"  class=" btn btn-lg" ng-show="showBtn==0"  >
           <h4 style="background-color:#4CF79E; padding:2px;"> গুরুত্বপূর্ণ নির্দেশাবলী  </h4>
        <ol>
            <li>রিফ্রেশ বা পৃষ্ঠাটি পুনরায় লোড এবং Backword বাটনে ক্লিক করা নিষেধ  </li>
            <li> প্রতি একক প্রশ্নের জন্য শুধুমাত্র একটি সুযোগ  </li>
            <li> সময় পেজ এর ডান  পাশে প্রদর্শিত হবে </li>
        </ol>
       <button ng-click=" GetSearchQuestion(<?php echo $ExamID;?>);" onclick="openFullscreen();" style="color:white; background-color:black;"> আপনি কি প্রস্তুত? শুরু করার জন্য এখানে ক্লিক করুন.          
      </button> 
      </span>
...
<!-- Time -->
        ...
        <div style="" ng-show="ShowCheckResultBtn == 1" class="btn btn-danger myTimediv">
         
            <img data-ng-src="data:image/png;base64,<?php echo $_SESSION['Photo']   ?>" width="100" height="100">
            <progress ng-value="time" max="120" ng-show="time<=120" >{{time}}</progress>
            <div style="font-size:20px; background-color:black;">Time Remain: </div> 
          <div style="font-size:25px;">{{time/ 60-0.5|number:0}} Min {{time% 60}} Sec </div>
          <hr>
           <div style="font-size:20px; background-color:black;">Total Selected </div> 
          <div style="font-size:25px;">{{TotalSelected}} </div>
          <hr>
             <div style="font-size:20px; background-color:black;">Unselected Yet </div> 
          <div style="font-size:25px;">{{AllQuestion.OnlyQuestion.length-TotalSelected}} </div>
...
<div style="position:fixed; top:340px; right: 50px;" ng-show="ShowCheckResultBtn == 1">
        ...
        <table class="table table-striped" style="margin-left:0px;">
            ...
            <tr ng-repeat="Item in AllQuestion.OnlyQuestion">
                <td style="padding-left:35px; "> {{$index + 1}}] <span style="padding: 5px; font-family: 'Times New Roman', Times, serif; font-size: 17px;  " class=""> {{Item.Question}} </span> 
                   <br><br>
                    <span  ng-click="Item.Click = 1" style="font-weight: bold; margin-left:23px;" ng-repeat="x in AllQuestion.AllQuestion| filter:{QId:Item.QId}">
                       
                        <span href="#" data-toggle="tooltip" data-placement="right"  ng-hide="Item.Click == 1" ng-model="x.Selected"  title="Click after being sure..!" ng-click="x.Selected = 1; TakeAns(x)"  style="margin:3px; background-color:yellow; color:black;"  class="btn btn-default"> 
                          {{getLetter($index)}}. &nbsp;&nbsp;  {{x.MCQ}}
                        </span>
                        <br>
                    </span>
                </td>
            ...
            <tr>
                ...
                <button type="button" ng-show="ShowCheckResultBtn == 1" ng-click="CheckResult(1)" class="btn btn-info pull-left" > পরীক্ষা সাবমিট করুন  </button> 
<br><br><br><br>
                </td>
    ...
    app.controller("CreateQuestionCtrl", ["$scope", "$http", "$timeout",
        function ($scope, $http, $timeout) {
            ...
            function CheckResult(value)
            {
                var r=true;
                if(value==1){
                var r=confirm("পরীক্ষা সাবমিট   করার ব্যাপারে  আপনি কি নিশ্চিত ??");
                }
