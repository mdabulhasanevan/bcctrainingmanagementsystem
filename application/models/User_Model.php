...
//    menu show in admin panel
    public function AdminMenu($user) {
        ...
        if ($dataIn == NULL) { $where=0;} else {$where=$dataIn->MenuID;}
        
        //sub Menu
        $this->db->where(array('UserID' => $user->Id,'WhatMenu'=>2));
        $dataIn2 = $this->db->get('menuroll')->row();
        if ($dataIn2 == NULL) { $where2=0; } else { $where2=$dataIn2->MenuID;}
        ...
        $userID=$_SESSION["id"];
        $QuickMenu=$this->db->query("SELECT am1.MenuName, COUNT(vh.ID) as visit, vh.* , max(vh.Date) as time FROM VisitorHistory as vh RIGHT JOIN adminsubmenu1 am1 on am1.Url=SUBSTRING(vh.URI,2,length(vh.URI)) WHERE vh.User=$userID group by vh.URI ORDER by time desc limit 10")->result();
    ...
    public function LoginHistory($query) {
      ...
      // Student is login
    public function StudentLoginHistory($query) {
    ...
    public function StudentExamAttendHistory($ExamID,$GEO)
    {
        $this->load->helper('date');
        $data = array(
        ...
        $this->db->insert('StudentExamAttendHistory', $data);
    ...
    public function GetStudentLoginHistory() {
        $result = $this->db->query("SELECT slh.*, st.Name, b.Batch,st.RegNO FROM StudentLoginHistory slh left JOIN student st on st.SID=slh.SID left JOIN batch b on st.BatchID=b.Id order by ID desc LIMIT 200")->result();

        return $result;
    }
    
     public function GetPageVisitLoginHistory($id) {
       // $result = $this->db->query("SELECT VH.URI, COUNT(VH.ID) as Visit, ((SELECT count(URI) from VisitorHistory)) as Total FROM VisitorHistory as VH GROUP BY VH.URI order by Visit desc")->result();
        $result = $this->db->query("SELECT VH.ID, r.Name, VH.URI, VH.Date, VH.user, VH.IP FROM VisitorHistory as VH LEFT JOIN registration r on r.Id=VH.User where VH.User=$id  order by VH.ID desc LIMIT 200")->result();
        return $result;
    }
    ...
    public function GetTeacherUser() {
        $query = $this->db->query("SELECT registration .*, role_tbl.Role as RoleName, post.PostName as PostName FROM registration LEFT join role_tbl on registration.Role=role_tbl.Id LEFT join post on registration.Post=post.PId  ORDER by registration.Post ASC");
        $rows = $query->result();
        return $rows;
    }
...
//    geting user or User all are same
    public function GetUser() {
        ...
        $userID = $_SESSION["id"];
        $query = $this->db->query("SELECT registration .*, role_tbl.Role as RoleName, post.PostName as PostName FROM registration LEFT join role_tbl on registration.Role=role_tbl.Id LEFT join post on registration.Post=post.PId WHERE registration.Id='$userID' ORDER by registration.Post ASC");
        $rows = $query->result();
        return $rows;
    }
...
//for select user role for any user
    public function LoadAllMenuAndUserRole($id) {
        ...
        if ($dataIn1 == NULL) { $where1=0; } else { $where1=$dataIn1->MenuID;}
        if ($dataIn2 == NULL) { $where2=0; } else { $where2=$dataIn2->MenuID;}
        $data = array(
